...
#define MICHI "city011"		//未知のデータ
#define KIJUN "city022"		//既知のデータ
#define VER "ver3_3_2"
...
double min4(double num0,double num1,double num2,double num3){
	if(num0<num1){
		if(num0<num2){
			if(num0<num3){
				return num0;
			...
			if(i==0 && j==0){
				...
				if(i==0){
					...
					if(j==0){
						...
						if(i>1&&j>1){
							num0 = g[i-1][j] + d[i][j];
							num1 = g[i-1][j-1] + sqrt(d[i][j]);
							num2 = g[i-1][j-2] + d[i][j];
							num3 = g[i-2][j-1] + d1[i][j];
							g[i][j] = min4(num0,num1,num2,num3);
						}else{
							num0 = g[i-1][j] + d[i][j];
